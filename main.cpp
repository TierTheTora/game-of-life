...
char helps[][1024] = {
   ...
   "speed(=<int>) : Set the amount of time it takes for a generation to complete (ms) | Write the current speed\n",
   "size(=<int>,<int>) : Clear and set the size of the the board (x,y) | Write the current size\n",
   ...
   "wrap(=<on/off>) : Allow or disallow cells to wrap around the board (may cause issues) | Writes if wrap is on or off\n",
   ...
   "gen <int> : Set the current generation\n"
};
uint8_t            ROWS       = 20;
uint8_t            COLS       = 20;
uint8_t            OLD_ROWS   = 20;
